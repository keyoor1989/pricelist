...
export default function AddProductModal({ brands, models, categories, onClose, onSave, isLoading = false }) {
  ...
  useEffect(() => {
    if (formData.brandId) {
      setFilteredModels(models.filter(model => model.brandId === formData.brandId));
      
      // Reset model selection if it doesn't belong to selected brand
      if (formData.modelId) {
        const modelExists = models.some(
          model => model.id === formData.modelId && model.brandId === formData.brandId
        );
  ...
  const handleChange = (e) => {
    ...
    if (['purchasePrice', 'dealerPrice', 'endUserPrice', 'gst'].includes(name)) {
